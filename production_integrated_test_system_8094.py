...
# sys.path.insert(0, '/root/projects/tencent-doc-manager/测试版本-性能优化开发-20250811-001430')  # 已确认不需要，所有模块都在主目录或production目录中
...
try:
    ...
    MODULES_STATUS['integrated_scorer'] = True
    logger.info("✅ 成功导入 integrated_scorer")
except ImportError as e:
    logger.warning(f"⚠️ 无法导入 integrated_scorer: {e}")
    MODULES_STATUS['integrated_scorer'] = False

try:
    from production.core_modules.l2_semantic_analysis_two_layer import L2SemanticAnalyzer
    MODULES_STATUS['l2_analyzer'] = True
    logger.info("✅ 成功导入 l2_semantic_analysis_two_layer")
except ImportError as e:
    logger.warning(f"⚠️ 无法导入 l2_semantic_analysis_two_layer: {e}")
    MODULES_STATUS['l2_analyzer'] = False

try:
    from production.core_modules.column_standardization_prompt import ColumnStandardizer
    MODULES_STATUS['column_standardizer'] = True
    logger.info("✅ 成功导入 column_standardization_prompt")
except ImportError as e:
    logger.warning(f"⚠️ 无法导入 column_standardization_prompt: {e}")
    MODULES_STATUS['column_standardizer'] = False
    ...
    <div class="container">
        ...
        <div class="main-grid">
            ...
            <div class="card full-width" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
                ...
                <div class="form-group">
                    <label style="color: white;">批量输入（每行一对URL，用逗号、空格或竖线分隔）</label>
                    <textarea id="batch-input" rows="8" placeholder="输入格式示例：
基线URL1, 目标URL1
基线URL2 | 目标URL2
https://docs.qq.com/sheet/xxx, https://docs.qq.com/sheet/yyy

# 支持注释行（以#开头）
# 空行会自动忽略" style="font-family: monospace; background: rgba(255,255,255,0.9); color: #333;"></textarea>
                </div>
                <div style="display: flex; gap: 10px;">
                    <button class="btn" style="background: white; color: #667eea;" onclick="startBatchProcess()" id="batch-btn">
                        开始批量处理
                    </button>
                    <button class="btn btn-danger" onclick="cancelBatchProcess()" id="batch-cancel-btn" style="display: none;">
                        取消批量处理
                    </button>
                    <button class="btn" style="background: rgba(255,255,255,0.2); border: 1px solid white;" onclick="clearBatchInput()">
                        清空输入
                    </button>
                </div>
                <div id="batch-progress" style="margin-top: 15px; display: none;">
                    <div style="background: rgba(255,255,255,0.2); padding: 10px; border-radius: 5px;">
                        <div style="margin-bottom: 5px;">处理进度：<span id="batch-progress-text">0/0</span></div>
                        <div style="background: rgba(255,255,255,0.3); height: 20px; border-radius: 10px; overflow: hidden;">
                            <div id="batch-progress-bar" style="background: white; height: 100%; width: 0%; transition: width 0.3s;"></div>
                        </div>
                    </div>
                </div>
                <div id="batch-results" style="margin-top: 15px; display: none;">
                    <div style="background: rgba(255,255,255,0.95); color: #333; padding: 15px; border-radius: 8px;">
                        <h4 style="margin-top: 0;">📊 批量处理结果</h4>
    ...
    <script>
        ...
        function startComparison() {
            ...
            .then(response => {
                // clearTimeout(timeoutId); // 已移除超时，不需要清除
                if (!response.ok) {
                    // 特殊处理502错误 - 这通常是nginx超时，但后端可能还在处理
                    if (response.status === 502) {
                        addLog('⚠️ 收到502响应，但后端可能仍在处理中...', 'warning');
                        addLog('💡 这通常是因为下载耗时较长（约90秒），请等待...', 'info');
                        addLog('🔄 后端处理通常会成功完成，请稍后查看结果', 'info');
                        // 等待一段时间后尝试获取最新结果
                        setTimeout(() => {
                            addLog('🔍 尝试获取处理结果...', 'info');
                            loadLatestResult();
                        }, 5000);
                        // 返回一个模拟的成功响应，避免显示错误
                        return {
                            success: true,
                            warning: '由于处理时间较长，请等待后台完成',
                            delayed: true
                        };
                    }
                            ...
                            fetch('/api/latest-result')
                                ...
                                .then(res => {
                                    if (res.success && res.result) {
                                        ...
                                        addLog('✅ 后台处理完成！', 'success');
                                        addLog(`📊 总变更数: ${res.result.total_changes}`, 'info');
                                        addLog(`📈 相似度: ${(res.result.similarity_score * 100).toFixed(2)}%`, 'info');
                                        displayComparisonResult(res.result);
                                    }
                                });
                            if (checkCount >= 20) { // 最多检查20次（100秒）
                                clearInterval(checkInterval);
                                addLog('✅ 后台处理应该已完成，请手动刷新查看结果', 'info');
                            }
                        }, 5000); // 每5秒检查一次
                    } else {
        ...
        function startBatchProcess() {
            ...
            document.getElementById('batch-btn').textContent = '批量处理中...';
            ...
            .then(data => {
                if (data.success) {
                    ...
                    addLog(`✅ 批量处理完成: ${summary.success_count}/${summary.total_tasks} 成功`, 'success');
                    
                    // 显示结果
                    showBatchResults(summary);
                } else {
                    addLog(`❌ 批量处理失败: ${data.error}`, 'error');
                }
            })
            .catch(error => {
                if (error.name !== 'AbortError') {
                    addLog(`❌ 批量处理出错: ${error}`, 'error');
                }
            })
            .finally(() => {
                ...
                document.getElementById('batch-btn').textContent = '开始批量处理';
        ...
        function updateBatchProgress() {
            // 这里可以通过WebSocket或轮询来获取实时进度
            // 暂时使用模拟进度
            let progress = 0;
            const interval = setInterval(() => {
                if (!isBatchRunning || progress >= 90) {
        ...
        function showBatchResults(summary) {
            const resultsDiv = document.getElementById('batch-results');
            const contentDiv = document.getElementById('batch-results-content');
            
            let html = `
                <div style="margin-bottom: 15px;">
                    <strong>总任务数:</strong> ${summary.total_tasks}<br>
                    <strong>成功:</strong> <span style="color: #27ae60;">${summary.success_count}</span><br>
                    <strong>失败:</strong> <span style="color: #e74c3c;">${summary.failed_count}</span><br>
                    <strong>完成时间:</strong> ${summary.timestamp}
                </div>
            ...
            summary.results.forEach(result => {
                ...
                const color = result.success ? '#27ae60' : '#e74c3c';
                html += `
                    <div style="padding: 8px; margin-bottom: 5px; background: #f8f9fa; border-left: 3px solid ${color};">
                        ${status} 任务 ${result.task_index}: 
                        ${result.success ? `相似度 ${(result.result.comparison?.similarity_score || 0) * 100}%` : result.error}
                    </div>
                `;
            });
...
def execute_full_pipeline(baseline_url, target_url, cookie, task_index=1, total_tasks=1):
    """执行完整的处理流程：下载、对比、AI分析、评分"""
    try:
        logger.info(f"\n{'='*70}")
        logger.info(f"🚀 任务 {task_index}/{total_tasks}: 执行完整处理流程")
        logger.info(f"{'='*70}")
        
        # 步骤1：下载和CSV转换
        logger.info("📥 步骤1: 下载文档并转换为CSV")
        
        # 下载基线文档
        baseline_success, baseline_file, baseline_error = downloader.download_document(
            baseline_url, cookie, "csv"
        )
        if not baseline_success:
            raise Exception(f"基线文档下载失败: {baseline_error}")
        
        # 下载目标文档
        target_success, target_file, target_error = downloader.download_document(
            target_url, cookie, "csv"
        )
        if not target_success:
            raise Exception(f"目标文档下载失败: {target_error}")
        
        # 步骤2：CSV对比分析
        logger.info("📊 步骤2: 执行CSV对比分析")
        comparison_result = comparator.compare_files(baseline_file, target_file)
        
        if not comparison_result:
            raise Exception("CSV对比失败")
        
        # 步骤3：AI列名标准化（如果模块可用）
        standardized_result = comparison_result
        if MODULES_STATUS.get('column_standardizer'):
            try:
                logger.info("🤖 步骤3: AI列名标准化")
                standardizer = ColumnStandardizer()
                standardized_result = standardizer.standardize_comparison(comparison_result)
            except Exception as e:
                logger.warning(f"列名标准化失败，使用原始结果: {e}")
        else:
            logger.info("⏭️ 步骤3: 跳过列名标准化（模块不可用）")
        
        # 步骤4：L2语义分析（如果模块可用）
        l2_score = None
        if MODULES_STATUS.get('l2_analyzer'):
            try:
                logger.info("🧠 步骤4: L2语义分析评分")
                l2_analyzer = L2SemanticAnalyzer()
                # 提取修改项进行分析
                modifications = standardized_result.get('modifications', [])
                if modifications:
                    l2_result = l2_analyzer.analyze_modifications(modifications)
                    l2_score = l2_result.get('average_confidence', 0)
                    standardized_result['l2_analysis'] = l2_result
                else:
                    logger.info("没有找到修改项，跳过L2分析")
            except Exception as e:
                logger.warning(f"L2语义分析失败: {e}")
        else:
            logger.info("⏭️ 步骤4: 跳过L2语义分析（模块不可用）")
        
        # 步骤5：综合评分（如果模块可用）
        final_score = None
        if MODULES_STATUS.get('integrated_scorer'):
            try:
                logger.info("📈 步骤5: 综合评分计算")
                scorer = IntegratedScorer(use_ai=False)  # 不使用AI避免重复L2分析
                # 为每个修改项打分
                modifications = standardized_result.get('modifications', [])
                scored_mods = []
                total_score = 0
                if modifications:
                    for i, mod in enumerate(modifications):
                        mod_score = scorer.score_modification(mod, f"mod_{i}")
                        scored_mods.append(mod_score)
                        total_score += mod_score.get('weighted_score', 0)
                    
                    final_score = total_score / len(modifications) if modifications else 0
                    standardized_result['scoring'] = {
                        'total_score': final_score,
                        'scored_modifications': scored_mods
                    }
                else:
                    logger.info("没有找到修改项，跳过综合评分")
            except Exception as e:
                logger.warning(f"综合评分失败: {e}")
        else:
            logger.info("⏭️ 步骤5: 跳过综合评分（模块不可用）")
        
        # 步骤6：保存结果
        logger.info("💾 步骤6: 保存处理结果")
        result_file = COMPARISON_RESULTS_DIR / f"full_pipeline_{datetime.now().strftime('%Y%m%d_%H%M%S')}_task{task_index}.json"
        ...
        full_result = {
            ...
            'comparison': standardized_result,
            'l2_score': l2_score,
        ...
        logger.info(f"✅ 任务 {task_index}/{total_tasks} 完成")
        logger.info(f"   相似度: {standardized_result.get('similarity_score', 0):.2%}")
        if l2_score is not None:
            logger.info(f"   L2评分: {l2_score:.2f}")
        if final_score is not None:
            logger.info(f"   综合评分: {final_score:.2f}")
        
        return full_result
        
    except Exception as e:
        logger.error(f"❌ 任务 {task_index} 执行失败: {e}")
...
def api_batch_process():
    ...
    try:
        data = request.json
        batch_input = data.get('batch_input', '')
        
        # 从请求中获取cookie，如果没有则从配置文件加载
        cookie = data.get('cookie', '')
        if not cookie:
            config = config_manager.load_config()
            cookie = config.get('cookie', '')
        ...
        if not batch_input:
            return jsonify({'success': False, 'error': '批量输入不能为空'})
        ...
        for line in lines:
            ...
            # 支持多种格式：URL1,URL2 或 URL1|URL2 或 URL1 URL2
            parts = line.replace(',', '|').replace(' ', '|').split('|')
            if len(parts) >= 2:
                baseline_url = parts[0].strip()
        ...
        if not tasks:
            return jsonify({'success': False, 'error': '未找到有效的URL对'})
        ...
        for i, task in enumerate(tasks, 1):
            try:
                logger.info(f"📊 处理任务 {i}/{len(tasks)}: {task['baseline_url']} vs {task['target_url']}")
                ...
                results.append({
                    'task_index': i,
                    'baseline_url': task['baseline_url'],
                ...
                logger.error(f"❌ 任务 {i} 失败: {task_error}")
                results.append({
                    'task_index': i,
                    'baseline_url': task['baseline_url'],
                    'target_url': task['target_url'],
        ...
        # 保存批量处理结果
        batch_result_file = COMPARISON_RESULTS_DIR / f"batch_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        return jsonify({
            'success': True,
            'summary': summary,
            'result_file': str(batch_result_file.name)
        })
