...
支持pandas和标准库双模式，增强环境兼容性
"""

# 优雅的依赖导入 - 支持pandas或标准库降级
try:
    import pandas as pd
    import numpy as np
    PANDAS_AVAILABLE = True
    print("✅ Pandas可用 - 使用完整功能模式")
except ImportError:
    PANDAS_AVAILABLE = False
    print("⚠️ Pandas不可用 - 使用标准库兼容模式")
...
def safe_isna(value) -> bool:
    ...
    if PANDAS_AVAILABLE:
        ...
        if isinstance(value, float) and str(value).lower() == 'nan':
            return True
        if isinstance(value, str) and value.strip() in ['', 'nan', 'NaN', 'NULL', 'null']:
            return True
        return False
...
平均置信度: {safe_mean(list(result["confidence_scores"].values())):.3f}
...
class AdaptiveDataStandardizer:
    ...
    def _standardize_datetime(self, value: Any, column_name: str) -> Optional[str]:
        """标准化日期时间"""
        if not value or safe_isna(value):
            return None
    ...
    def adaptive_compare_tables(self, current_tables: List[Dict], 
        ...
        for table_index, table_info in enumerate(current_tables):
            try:
                ...
                # 更新统计信息
                avg_confidence = safe_mean(list(matching_result["confidence_scores"].values())) \
...
"""
    ...
    def _generate_quality_summary(self, comparison_results: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            "average_quality_score": safe_mean(quality_scores) if quality_scores else 0.0,
