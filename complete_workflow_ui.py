            ...
            <div class="step-card" id="step-4">
                ...
                <div class="form-group">
                    ...
                    <div class="btn-group" style="display: flex; gap: 10px; margin-bottom: 15px;">
                        <button class="btn btn-secondary" onclick="switchUploadMode('select')" id="modeSelect">
                            📂 选择已下载文件
                        </button>
                        <button class="btn btn-secondary" onclick="switchUploadMode('manual')" id="modeManual">
                            ✏️ 手动输入路径
                        </button>
                    </div>
                </div>
                ...
                <!-- 选择文件模式 -->
                <div id="uploadModeSelect" class="form-group">
                ...
                <!-- 手动输入模式 -->
                <div id="uploadModeManual" class="form-group" style="display:none;">
                    <label>文件路径</label>
                    <input type="text" id="uploadFilePath" class="form-control"
                        placeholder="输入完整文件路径，如: /root/projects/test.xlsx">
                    <small class="text-muted">支持绝对路径或相对路径</small>
                </div>
                
                <div class="form-group">
                    <label>🔑 上传专用Cookie <span style="color: #dc2626;">(必填)</span></label>
                    <textarea id="uploadCookie" class="form-control" rows="3"
                        placeholder="粘贴从腾讯文档获取的Cookie字符串...
格式: uid=xxx; DOC_SID=xxx; SID=xxx; TOK=xxx..."></textarea>
                    <small class="text-muted">Cookie获取方法：登录腾讯文档 → F12 → Network → 刷新页面 → 找到docs.qq.com请求 → Headers → Cookie</small>
                </div>
    ...
    <script>
        ...
        function updateUploadFileSelect() {
            ...
            // 添加测试文件选项
            uploadSelect.innerHTML += '<option value="/root/projects/tencent-doc-manager/excel_outputs/risk_analysis_report_20250819_024132.xlsx">[测试] risk_analysis_report.xlsx</option>';
            
            // 允许上传修改后的文件
            modifiedFiles.forEach(file => {
                uploadSelect.innerHTML += `<option value="${file.path}">[修改] ${file.name}</option>`;
            });
            
            // 也允许直接上传下载的文件
            downloadedFiles.forEach(file => {
                uploadSelect.innerHTML += `<option value="${file.path}">[原始] ${file.name}</option>`;
            });
        }
        ...
        function switchUploadMode(mode) {
            if (mode === 'select') {
                document.getElementById('uploadModeSelect').style.display = 'block';
                document.getElementById('uploadModeManual').style.display = 'none';
                document.getElementById('modeSelect').classList.add('btn-primary');
                document.getElementById('modeSelect').classList.remove('btn-secondary');
                document.getElementById('modeManual').classList.remove('btn-primary');
                document.getElementById('modeManual').classList.add('btn-secondary');
            } else {
                document.getElementById('uploadModeSelect').style.display = 'none';
                document.getElementById('uploadModeManual').style.display = 'block';
                document.getElementById('modeManual').classList.add('btn-primary');
                document.getElementById('modeManual').classList.remove('btn-secondary');
                document.getElementById('modeSelect').classList.remove('btn-primary');
                document.getElementById('modeSelect').classList.add('btn-secondary');
            }
        }
        ...
        async function uploadDocument() {
            // 获取文件路径
            let filePath;
            const isManualMode = document.getElementById('uploadModeManual').style.display !== 'none';
            
            if (isManualMode) {
                filePath = document.getElementById('uploadFilePath').value.trim();
                if (!filePath) {
                    showAlert('请输入文件路径', 'error');
                    return;
                }
            } else {
                filePath = document.getElementById('uploadFileSelect').value;
                if (!filePath) {
            ...
            // 获取Cookie
            const cookieString = document.getElementById('uploadCookie').value.trim();
            if (!cookieString) {
                showAlert('请输入上传专用Cookie', 'error');
                return;
            }
            ...
            log(`使用Cookie: ${cookieString.substring(0, 50)}...`, 'info');
...
def upload_document():
    ...
    try:
        ...
        cookie_string = data.get('cookie_string', '')  # 接收前端传来的Cookie
        ...
        # 尝试使用真实的上传功能（生产级版本）
        try:
            # 优先使用生产级上传模块 v3（多策略链接匹配，包括文件名匹配和网络监控）
            from production.core_modules.tencent_doc_upload_production_v3 import sync_upload_v3 as sync_upload
            
            # 如果前端没有提供Cookie，尝试从配置文件读取
            ...
            # 执行真实上传
            print(f"[上传] 使用生产级模块上传文件: {file_path}")
            print(f"[上传] 上传选项: {upload_option}")
            ...
            print(f"[上传] 结果: {result}")
            
            # 添加上传选项信息
            if result.get('success'):
                if upload_option == 'replace' and target_url:
                    result['message'] = f"已上传新文档（原URL: {target_url}）"
                    result['original_url'] = target_url
            
            return jsonify(result)
            
        except ImportError as e:
            print(f"[上传] 生产级模块不可用: {e}")
            # 尝试使用备用模块
            try:
