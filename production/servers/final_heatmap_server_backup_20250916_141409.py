          ...
          React.useEffect(() => {
            ...
            data.forEach((row, rowIndex) => {
              ...
              row.forEach((value, colIndex) => {
                // 🎯 性能优化：使用修改掩码判断是否需要作为热源
                const isModified = modificationMask && modificationMask[rowIndex] && modificationMask[rowIndex][colIndex];

                // 只把有修改的格子作为热源，大幅减少计算量
                if (typeof value === 'number' && !isNaN(value)) {
                  if (isModified || value > 0.05) {  // 有修改标记或值明显大于背景值
                    heatSources.push({
            ...
            const reductionRate = totalCells > 0 ? ((totalCells - heatSources.length) / totalCells * 100).toFixed(1) : 0;
            console.log(`🔥 提取 ${heatSources.length} 个真实热源点`);
            console.log(`🎯 性能优化: 从 ${totalCells} 个格子减少到 ${heatSources.length} 个热源 (减少${reductionRate}%)`);
            ...
            const maxInfluenceDistance = Math.max(cellWidth, cellHeight) * 1.2; // 大幅减小影响距离，避免过度模糊扩散
            ...
            for (let y = 0; y < canvasHeight; y++) {
              for (let x = 0; x < canvasWidth; x++) {
                ...
                // 计算最终的插值强度，设置默认背景值0.05
                let finalIntensity = 0.05; // 设置默认背景值
                if (weightSum > 0) {
                  finalIntensity = Math.max(0.05, weightedSum / weightSum); // 确保不低于背景值
                } else if (closestSource && minDistance <= maxInfluenceDistance * 1.5) {
                  // 距离衰减（减小影响范围）
                  const decay = Math.exp(-minDistance / (maxInfluenceDistance * 0.3));
                  finalIntensity = Math.max(0.05, closestSource.intensity * decay * 0.2);
                }
                ...
                // 确保所有单元格都显示，包括背景值0.05
                imageData.data[index + 3] = color.a;  // 所有单元格都不透明，显示背景色
              }
            }
            ...
            // ctx.filter = 'blur(1px)'; // 移除模糊效果，保持清晰度
            ctx.drawImage(canvas, 0, 0);
            
            // 增强对比度
            ctx.globalCompositeOperation = 'overlay';
            ctx.globalAlpha = 0.1;
            ctx.filter = 'contrast(110%)'; // 略微降低对比度增强
            ctx.drawImage(canvas, 0, 0);
