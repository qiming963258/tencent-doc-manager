...
## ⚠️ 已知问题与注意事项（2025-09-19更新）

### 1. 下载系统架构差异
- **文档描述**：技术架构文档提到使用`PlaywrightDownloader`类
- **实际实现**：系统使用`TencentDocAutoExporter`类（位于`production/core_modules/tencent_export_automation.py`）
- **影响**：功能正常工作，但文档与实现不一致
- **建议**：使用`TencentDocAutoExporter`进行所有下载操作

### 2. 虚假变更问题（显示2010处变更）
- **症状**：系统总是显示"发现2010处变更"，即使实际变更很少
- **根本原因**：
  1. 系统对比了不同的文档（如：出国销售计划表 vs 小红书部门）
  2. 测试数据文件`generate_standard_comprehensive_score.py`硬编码了2010行
  3. 基线文档和目标文档不匹配

- **正确的对比配置**：
  ```
  ✅ 正确：基线(出国销售v1) vs 目标(出国销售v2) - 同一文档的不同版本
  ❌ 错误：基线(出国销售) vs 目标(小红书部门) - 不同的文档
  ```

- **解决方案**：
  1. 确保下载相同文档进行版本对比
  2. 检查URL配置，基线和目标应该指向同一文档
  3. 避免使用测试数据生成脚本
  4. 验证实际文件行数：`wc -l filename.csv`

### 3. Cookie有效期问题
- **有效期**：7-14天
- **更新方法**：在`config/cookies.json`更新
- **最后更新**：2025-09-19（查看文件时间戳验证）

### 4. 实际文件与显示不符
- **例如**：小红书CSV文件实际1474行，但系统可能显示2010行
- **原因**：混用了测试数据和真实数据
- **验证方法**：使用`wc -l`命令检查实际行数
